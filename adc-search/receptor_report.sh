...
python3 ${SCRIPT_DIR}/adc-search.py ${REPOSITORY_TSV} ${REPERTOIRE_QUERY_JSON} $JSON_QUERY_FILE --service_delay=0.2 --output_file=$OUTPUT_DIR/all-$CDR3.json --field_file=${REPERTOIRE_FIELD_TSV} --verbose > $OUTPUT_DIR/all-$CDR3.out

# Generate a report for the repertoires where the receptor was found

COUNT=`cat $OUTPUT_DIR/all-$CDR3.json | jq '[.[].results[].Facet[].count]| add | if . > 0 then . else 0 end'`
...
echo "Found TCR receptors specific to $CDR3 in IEDB:" >> $REPORT_FILE
...
echo "Found BCR receptors specific to $CDR3 in IEDB:" >> $REPORT_FILE
