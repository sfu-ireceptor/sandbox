...
find "$receptor_file_path" -maxdepth 1 -type f | while read -r file; do
	...
	echo "../../run_receptor_reports_fast.sh ${receptor_file_path}/${base_name}.tsv repository-t1d-1.tsv repertoire-tr-contains-schema-rearrangement.json repertoire_fields.tsv $base_name > $base_name.out"
